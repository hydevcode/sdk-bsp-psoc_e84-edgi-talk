...
#if defined(COMPONENT_MTB_HAL)
#define CYBSP_WIFI_WL_REG_ON_GPIO_DRIVE_MODE    (MTB_HAL_GPIO_DRIVE_PULLUP)
#else
...
#if defined(COMPONENT_MTB_HAL)
#define CYBSP_WIFI_WL_HOSTWAKE_DRIVE_MODE        (MTB_HAL_GPIO_DRIVE_NONE)
#else
...
#if !defined(CYHAL_API_VERSION) && !defined(COMPONENT_MTB_HAL)
...
//--------------------------------------------------------------------------------------------------
static void _cybsp_wifi_reset_wifi_chip(mtb_hal_gpio_t* reset_pin)
{
    mtb_hal_gpio_write(reset_pin, false);
    // WiFi into reset
    // Allow CBUCK regulator to discharge
    (void)mtb_hal_system_delay_ms(WLAN_CBUCK_DISCHARGE_MS);
    // WiFi out of reset
    mtb_hal_gpio_write(reset_pin, true);
    (void)mtb_hal_system_delay_ms(WLAN_POWER_UP_DELAY_MS);
}
...
//--------------------------------------------------------------------------------------------------
#if defined(COMPONENT_MTB_HAL)
static cy_rslt_t _cybsp_wifi_sdio_try_send_cmd(mtb_hal_sdio_t* sdio_object,
                                               ...
                                               uint32_t* response)
{
    ...
    do
    {
        result = mtb_hal_sdio_host_send_cmd(sdio_object, direction, command, argument, response);
        if (result != CY_RSLT_SUCCESS)
        {
            mtb_hal_system_delay_ms(SDIO_RETRY_DELAY_MS);
        }
        loop_count++;
...
//--------------------------------------------------------------------------------------------------
#if defined(COMPONENT_MTB_HAL)
static cy_rslt_t _cybsp_wifi_sdio_card_init(mtb_hal_sdio_t* sdio_object)
{
    cy_rslt_t result;
    ...
    do
    {
        ...
        result = _cybsp_wifi_sdio_try_send_cmd(sdio_object, MTB_HAL_SDIO_XFER_TYPE_WRITE,
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            result = _cybsp_wifi_sdio_try_send_cmd(sdio_object, MTB_HAL_SDIO_XFER_TYPE_READ,
                    ...
                    if (response & SDIO_CMD5_RESP_IORDY)
                    {
                        ...
                        if (response & SDIO_CMD5_S18R_BIT)
                        {
                            ...
                            if (CY_RSLT_SUCCESS == result)
                            {
                                ...
                                result = mtb_hal_sdio_host_set_io_voltage(sdio_object,
                                                                          MTB_HAL_SDIO_IO_VOLTAGE_1_8V,
                                                                          MTB_HAL_SDIO_IO_VOLT_ACTION_SWITCH_SEQ_ONLY);
                                if (MTB_HAL_SDIO_RSLT_ERR_UNSUPPORTED == result)
                                {
    ...
    if (result == CY_RSLT_SUCCESS)
    {
        mtb_hal_sdio_cfg_t config = { .frequencyhal_hz = sdio_frequency, .block_size = 0 };
        result = mtb_hal_sdio_configure(sdio_object, &config);
    }
    return result;
}
...
//--------------------------------------------------------------------------------------------------
#if defined(COMPONENT_MTB_HAL)
static cy_rslt_t _cybsp_wifi_spi_init_bus(mtb_hal_spi_t* spi_obj, mtb_hal_gpio_t* host_wake_pin)
{
    cy_rslt_t rslt = mtb_hal_spi_set_frequency(&spi_obj, 50000000); // 50 MHz operation
    if (CY_RSLT_SUCCESS == rslt)
    {
        whd_oob_config_t spi_oob_config =
        {
            .host_oob_pin      = host_wake_pin,
            .dev_gpio_sel      = DEFAULT_OOB_PIN,
            .is_falling_edge   = (CY_WIFI_HOST_WAKE_IRQ_EVENT == MTB_HAL_GPIO_IRQ_FALL)
                ? WHD_TRUE
                : WHD_FALSE,
            #if defined(WHD_OOB_CONFIG_VERSION) && ((WHD_OOB_CONFIG_VERSION) >= 2)
            .drive_mode        = CYBSP_WIFI_WL_HOSTWAKE_DRIVE_MODE,
...
#if defined(COMPONENT_MTB_HAL)
//--------------------------------------------------------------------------------------------------
// cybsp_wifi_init_primary_sdio
//--------------------------------------------------------------------------------------------------
cy_rslt_t cybsp_wifi_init_primary_sdio(whd_interface_t* interface,
                                       whd_init_config_t* init_config,
                                       whd_resource_source_t* resource_if,
                                       whd_buffer_funcs_t* buffer_if,
                                       whd_netif_funcs_t* netif_if,
                                       mtb_hal_sdio_t* sdio_obj,
                                       mtb_hal_gpio_t* host_wake_pin,
                                       mtb_hal_gpio_t* reset_pin)
{
    ...
    if (result == CY_RSLT_SUCCESS)
    {
        _cybsp_wifi_reset_wifi_chip(reset_pin);

        result = _cybsp_wifi_sdio_card_init(sdio_obj);
        if (result == CY_RSLT_SUCCESS)
        {
            whd_oob_config_t sdio_oob_config =
            {
                .host_oob_pin      = host_wake_pin,
                .dev_gpio_sel      = DEFAULT_OOB_PIN,
                .is_falling_edge   = (CY_WIFI_HOST_WAKE_IRQ_EVENT == MTB_HAL_GPIO_IRQ_FALL)
            ...
            whd_sdio_config_t whd_sdio_config =
            {
                .sdio_1bit_mode        = WHD_FALSE,
                .high_speed_sdio_clock = WHD_FALSE,
                .oob_config            = sdio_oob_config
            };
            result = whd_bus_sdio_attach(whd_drv, &whd_sdio_config, sdio_obj);
...
#if defined(COMPONENT_WIFI_INTERFACE_SPI)
//--------------------------------------------------------------------------------------------------
// cybsp_wifi_init_primary_spi
//--------------------------------------------------------------------------------------------------
cy_rslt_t cybsp_wifi_init_primary_spi(whd_interface_t* interface,
                                      whd_init_config_t* init_config,
                                      whd_resource_source_t* resource_if,
                                      whd_buffer_funcs_t* buffer_if,
                                      whd_netif_funcs_t* netif_if,
                                      ...
                                      mtb_hal_gpio_t* reset_pin)
{
    ...
    if (result == CY_RSLT_SUCCESS)
    {
        ...
        result = _cybsp_wifi_spi_init_bus(spi_obj, host_wake_pin);
...
#endif // if defined(COMPONENT_WIFI_INTERFACE_SPI)

#if defined(COMPONENT_WIFI_INTERFACE_M2M) || defined(COMPONENT_WIFI_INTERFACE_OCI)
//--------------------------------------------------------------------------------------------------
// cybsp_wifi_init_primary_extended
//--------------------------------------------------------------------------------------------------
cy_rslt_t cybsp_wifi_init_primary_extended_m2m_or_oci(whd_interface_t* interface,
                                                      whd_init_config_t* init_config,
                                                      whd_resource_source_t* resource_if,
                                                      whd_buffer_funcs_t* buffer_if,
                                                      whd_netif_funcs_t* netif_if)
{
    ...
    {
        #if defined(COMPONENT_WIFI_INTERFACE_M2M)
        result = _cybsp_wifi_m2m_init_bus();
        #elif defined(COMPONENT_WIFI_INTERFACE_OCI)
        result = _cybsp_wifi_custom_init_bus();
        #endif
...
#endif // defined(COMPONENT_WIFI_INTERFACE_M2M) || defined(COMPONENT_WIFI_INTERFACE_OCI)
#elif defined(CY_USING_HAL)
//--------------------------------------------------------------------------------------------------
// cybsp_wifi_init_primary_extended
//--------------------------------------------------------------------------------------------------
...
cy_rslt_t cybsp_wifi_deinit(whd_interface_t interface)
{
    ...
    if (result == CY_RSLT_SUCCESS)
    {
        ...
        #if defined(CYBSP_WIFI_SDIO_NEEDS_INIT) && !defined(COMPONENT_MTB_HAL)
        if (result == CY_RSLT_SUCCESS)
        {
            cyhal_sdio_free(&sdio_obj);
        }
